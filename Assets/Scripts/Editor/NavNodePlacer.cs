// This file is inside Assets/Scripts/Editor/ and is automatically excluded from builds.
#if UNITY_EDITOR
using UnityEngine;
using UnityEditor;
using ARLibraryNav.Navigation;

namespace ARLibraryNav.Editor
{
    /// <summary>
    ...
    ///
    /// Usage:
    ///   1. Create an empty GameObject in the scene (e.g. "NavGraphGizmos").
    ///   2. Add this component via Add Component > ARLibraryNav > Editor > NavNodePlacer.
    ///   3. Drag LibraryNavGraph.asset into the targetGraph field.
    ///   4. Open the Scene view nodes appear as coloured spheres, edges as lines.
    ///   5. Compare with the floor plan to verify node positions are correct.
    ///   6. DISABLE or DELETE this GameObject before building the APK.
    ///
    /// Node colours:
    ///   Cyan   = regular node
    ///   Yellow = stair/lift node
    ///
    /// Edge colours:
    ...
    public class NavNodePlacer : MonoBehaviour
    {
        ...
        private void OnDrawGizmos()
        {
            ...
            // Rebuild lookups in case asset was modified since last draw
            ...
            foreach (var node in targetGraph.nodes)
            {
                ...
                Vector3 pos = node.WorldPosition3D(nodeRadius); // Raise above floor
            ...
            foreach (var edge in targetGraph.edges)
            {
                ...
                Gizmos.DrawLine(fromNode.WorldPosition3D(nodeRadius), toNode.WorldPosition3D(nodeRadius));
